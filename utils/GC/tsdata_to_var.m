...
function [A,SIG,E] = tsdata_to_var(X,p,regmode,lambda)
...
if  strcmpi(regmode,'OLS') % OLS (QR decomposition)
    ...
    XXL = XL*XL' + lambda * eye(n);
    A = (X0*XL')/XXL;                     % OLS using QR decomposition
...
elseif strcmpi(regmode,'LWR') % LWR (Morf): patched, March 2018 to correct initialisation error (many thanks to Gonzalo Camba-Mendez for the heads-up)
    ...
    EE = reshape(X,n,N*m);
%     IC = inv(chol(cov(X'),'lower')); % inverse covariance square root
	...
	A0 = AF(:,1:n);
%     A0 = (AF(:,1:n)'*AF(:,1:n) + lambda*eye(n))\AF(:,1:n)';
